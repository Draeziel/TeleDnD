...
SHEET_RESOLVER_CUTOVER_ENABLED=false
SHEET_LEGACY_FALLBACK_ENABLED=true
```
...
- `SHEET_LEGACY_FALLBACK_ENABLED` – controls whether sheet build may fall back to legacy branch when resolver path is unavailable (`true` by default when cutover is off, `false` when cutover is on). In production this must remain `false`: startup fails fast with `startup_configuration_error` if legacy fallback is enabled.
