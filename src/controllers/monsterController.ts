...
export class MonsterController {
  ...
  public async updateTemplate(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { id } = req.params;
      const result = await this.monsterService.updateTemplate(telegramUserId, id, req.body || {});
      ...
      if (error instanceof Error && error.message.startsWith('Validation:')) {
        ...
        res.status(500).json({ message: 'Error updating monster template', error });
      }
    }
  }
  ...
  public async deleteTemplate(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { id } = req.params;
      const result = await this.monsterService.deleteTemplate(telegramUserId, id);
      res.status(200).json(result);
    } catch (error) {
      if (error instanceof Error && error.message.includes('Forbidden')) {
        ...
        res.status(500).json({ message: 'Error deleting monster template', error });
      }
    }
  }
}
