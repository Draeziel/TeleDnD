...
- 2026-02-20: Added integration smoke script `test-combat-automation.ps1` covering combat-action apply via template, immutable template snapshot, auto-tick decrement/expiry, and combat event payload checks.
 - 2026-02-20: Implemented backend `CharacterAssemblerService` and integrated assembly completeness validation into finalize flow. Finalize now calls assembler inside a transaction and rejects invalid assemblies. Added equipment slot/proficiency/attribute validations and seed-path assembly helpers.
 - 2026-02-20: Added unit/integration/golden tests for assembly (including `barbarian_lvl1_sheet.json` and `bard_lvl1_sheet.json`) and a `scripts/verifySheetGolden.ts` verifier. `verify:ci` script added to run build + unit + golden locally.
 - 2026-02-20: Golden verification included into smoke pipeline; local smoke run against compiled server passed: `Total: 28 | Passed: 28 | Failed: 0`.
 - 2026-02-20: Noted `ts-node-dev` dev-runner TypeScript resolution runtime error; temporary CI/smoke workaround uses compiled build. Follow-up task: investigate and fix dev-runner compile/runtime failure.
